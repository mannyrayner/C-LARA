...
generate a single image, appropriate to the story, which exemplifies the style.
...
Write the element names in {text_language}.
...
Write the brief summary in {text_language}.
...
- Write the prompt in {text_language}.
...
0. Write the description in {text_language}. 
...
A. Write the description in {text_language}. 

B. Remember that DALL-E-3 often fails to follow instructions, so the image may not represent the text well.

C. Be as precise and detailed as possible when producing your description of the image.
""",
...
A. Write the summary in {text_language}.

B. Look in particular at the section in the specification entitled "Essential aspects".
...
C. The response will be read by a Python script, so write only the single evaluation score followed by the summary, separated by a newline.
...
** Write the summary in {text_language} **
...
Write the descriptions in {text_language}
