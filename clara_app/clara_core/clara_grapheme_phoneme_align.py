...
from .clara_grapheme_phoneme_resources import get_grapheme_phoneme_alignments_for_key_and_resources, get_encoding
...
def dp_phonetic_align(Letters, Phonemes0, Resources):
    ...
    # If we're using an arpabet_like encoding, turn the phonemes into a list so that we have one element per phoneme.
    # There is however no need to turn the letters into a list even in this case.
    Phonemes = Phonemes0.split() if Encoding == 'arpabet_like' else Phonemes0
    ...
    for KeyL in KeysL:
        for KeyP in KeysP:
            ...
            # If we extend by skipping a character in either Letters or Phonemes (i.e. deletion/insertion), we charge 5 
                ...
                ( AlignedLetters, AlignedPhonemes, Cost ) = ( Letters[MatchLengthL], '' if Encoding == 'ipa' else [], 5 )
                extend1(Letters, Phonemes, MatchLengthL, MatchLengthP, AlignedLetters, AlignedPhonemes, Cost, DPDict)
            elif KeyL == '' and KeyP == 'skip':
                ( AlignedLetters, AlignedPhonemes, Cost ) = ( '', Phonemes[MatchLengthP] if Encoding == 'ipa' else [ Phonemes[MatchLengthP] ], 5 )
                extend1(Letters, Phonemes, MatchLengthL, MatchLengthP, AlignedLetters, AlignedPhonemes, Cost, DPDict)
            # If we skip in both (i.e. unknown substitution), we charge 5
            elif KeyL == 'skip' and KeyP == 'skip':
                ( AlignedLetters, AlignedPhonemes, Cost ) = ( Letters[MatchLengthL], Phonemes[MatchLengthP] if Encoding == 'ipa' else [ Phonemes[MatchLengthP] ], 5 )
                extend1(Letters, Phonemes, MatchLengthL, MatchLengthP, AlignedLetters, AlignedPhonemes, Cost, DPDict)
