...
def generate_segmented_version(text, l2_language):
    return generate_or_improve_segmented_version('annotate', text, l2_language)

# Improve annotation of the text with pages and segments
def improve_segmented_version(text, l2_language):
    return generate_or_improve_segmented_version('improve', text, l2_language)
...
# Improve annotation of the text with pages and segments
def improve_glossed_version(glossed_text, l1_language, l2_language):
    return generate_or_improve_annotated_version('improve', 'gloss', glossed_text, l1_language, l2_language)
...
def improve_tagged_version(tagged_text, l2_language):
    l1_language = 'irrelevant'
    return generate_or_improve_annotated_version('improve', 'lemma', tagged_text, l1_language, l2_language)

# Improve annotation of the text with lemmas and glosses
def improve_lemma_and_gloss_tagged_version(tagged_text, l1_language, l2_language):
    return generate_or_improve_annotated_version('improve', 'lemma_and_gloss', tagged_text, l1_language, l2_language)

def generate_or_improve_segmented_version(annotate_or_improve, text, l2_language):
...
def call_chatgpt4_to_annotate_or_improve_elements(annotate_or_improve, gloss_or_lemma, elements, l1_language, l2_language):
    ...
    elif gloss_or_lemma == 'lemma':
        annotation_prompt = tagging_prompt(annotate_or_improve, simplified_elements_json, l2_language)
    elif gloss_or_lemma == 'lemma_and_gloss' and annotate_or_improve == 'improve':
...
def annotated_string_to_simplified_internalised_form(example, l2_language, l1_language, gloss_or_lemma):
    internalised_example = clara_internalise.internalize_text(example, l2_language, l1_language, gloss_or_lemma)
...
def simplify_element_to_list(element, gloss_or_lemma):
    content = element.content
    annotation = element.annotations[gloss_or_lemma] if gloss_or_lemma in element.annotations else 'NO_ANNOTATION'
...
def unsimplify_element(element, gloss_or_lemma):
    if gloss_or_lemma == 'gloss' and len(element) == 2:
        content, gloss = element
        ...
        raise InternalCLARAError(message = f'Bad call: unsimplify_element({element}, {gloss_or_lemma})')

def parse_chatgpt_gloss_response(response, simplified_elements, gloss_or_lemma):
    try:
    ...
    for element in response_object:
        if not well_formed_element_in_glossing_or_tagging_response(element, gloss_or_lemma):
...
def well_formed_element_in_glossing_or_tagging_response(element, gloss_or_lemma):
    if gloss_or_lemma == 'gloss':
        return isinstance(element, list) and len(element) == 2
    elif gloss_or_lemma == 'lemma':
        return isinstance(element, list) and len(element) == 3
    elif gloss_or_lemma == 'lemma_and_gloss':
