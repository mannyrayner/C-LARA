...
from .clara_coherent_images_utils import get_page_params_from_project_params, default_params, get_element_image
...
def edit_images_v2(request, project_id, status):
    ...
    try:
        ...
        try:
            # New method, which expects that all annotated versions will be exactly aligned.
            # This in particular seems to be required for automatically created picture dictionary texts
            all_page_texts = clara_project_internal.get_page_texts()
        except:
            # Old inexact (but more robust) method
            messages.info(request, "Unable to align different annotated versions of the text exactly, falling back to approximate method")
            clara_project_internal.remove_any_empty_pages_at_end_and_save_for_all_text_versions()
            all_page_texts = clara_project_internal.get_page_texts_legacy()
    ...
    try:
        page_data = []
        if _edit_images_v2_trace:
            print(f'len(page_texts) = {len(page_texts)}')
            print(f'len(segmented_texts) = {len(segmented_texts)}')
        ...
        for index in range(0, len(page_texts)):
            ...
            if ( index < len(page_texts) and index < len(segmented_texts) and
                 ...
                 index < len(gloss_texts) and page in indexed_page_data ):
                item = { 'page': page,
        ...
        if _edit_images_v2_trace:
            print(f'len(page_data) = {len(page_data)}')
    except Exception as e:
        messages.error(request, f"Unable to get page information")
        messages.error(request, f"Exception: {str(e)}\n{traceback.format_exc()}")
        page_data = []
